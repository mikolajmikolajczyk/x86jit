...
- [x] **INT-T9** — Corpus ladder climbed to the interpreter summit: `sha256sum`/`wc` (busybox), musl `sha256sum`, **`sqlite3`** (in-memory query), **`lua`** (x87), and **`CPython 3.13`** (`python3 -S -c`, full bytecode VM). Also dynamically-linked musl + glibc hellos. **Further rungs done:** file-DB sqlite (writable passthrough, `tests/sqlite_file.rs`), **gzip/gunzip** (DEFLATE both ways, `tests/gzip.rs`). **Still open:** a larger python script (more stdlib), `git`/`perl`/`node`.

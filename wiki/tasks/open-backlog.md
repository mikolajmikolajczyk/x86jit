...
Instruction gaps the ladder keeps surfacing — **filled so far:** `bt*`, `cpuid`, `bsf`/`bsr`, `cwd`/`cdq`, `cbw`/`cwde`/`cdqe`, `pshuflw`/`pshufhw`, `pextrw`, `movhps`/`movlps`/`movlhps`/`movhlps`, the **x86-64-v2 (Jaguar) set** (`pshufb`, `popcnt`, `crc32`, `pextrb`, `pcmpgtq`/`pcmpeqq`), and the **x87 FPU** (f64-backed — true 80-bit precision deferred; raw `%Lf` output isn't bit-exact), and **`shld`/`shrd`** (double-precision shifts — surfaced by `busybox sort -n`, `tests/differential.rs::shld_shrd_match_unicorn` + `tests/busybox.rs::busybox_sort_numeric...`). **Still likely ahead:** the SSE4 tail (`palignr`, `pblendvb`, `ptest`, `pmovsx/zx`, `pmulld`, `pinsrb/d`), true-80-bit x87. Add each when a live path hits it, validated interp == JIT == Unicorn.

> **Forcing case for true-80-bit x87:** `busybox awk` `printf "%.6f", sqrt(2)` prints `1.414213` vs native `1.414214`. The double is bit-identical (`%.17g` matches); musl's `printf` formats via 80-bit `long double`, and our f64-backed x87 loses the guard bits, flipping the last-digit rounding. Concrete demonstration of why true-80-bit x87 matters.

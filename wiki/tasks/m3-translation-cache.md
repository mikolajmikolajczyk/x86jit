...
- [x] **M3-T3** — Confirm the interpreter `materialize` arm wraps `Arc<IrBlock>` and the cache is actually populated on first execution of each block. (§8, §9)
- [x] **M3-T4** — Hit/miss counters on the cache (for the test below and future stats). (§12 M3)
- [x] **M3-T5b** — Seam-2 marker (§17.4): keep the cache key `u64` (guest addr) today, but leave the `SEAM` comment noting it would become `BlockKey { guest_addr, mode }` if processor modes were ever added. Don't build `BlockKey` now. (§17.4, §17.6)

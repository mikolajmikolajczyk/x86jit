...
- Nothing active. Two backends agree on the corpus, the fuzzer, a vectorized SHA-256, a real musl libc program, a float program, self-modifying code, 8-way threading, a contended atomic counter, **real busybox** (`sha256sum`, `wc`), and **real sqlite3** (in-memory query). Live backlog: [`../tasks/open-backlog.md`](../tasks/open-backlog.md). **Next options:** keep climbing the ladder (`lua`/`python`, file-DB sqlite → surfaces `shld`/`shrd`, SSSE3/SSE4, x87), **dynamic linking** (`DYN-*`: ld.so, relocations, mmap/mprotect), or the **M7 TSO barrier tiers** (needs an ARM host).

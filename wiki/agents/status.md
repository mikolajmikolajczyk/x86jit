...
- **Dynamic linking (glibc).** The harder rung: `ld-linux` opens and **file-backed-mmaps `libc.so.6` at run time**, resolves versioned symbols, and starts the program — three ways (native==interp==JIT). Root-cause chain fixed (`fa2cbac`): fabricated `st_dev`/`st_ino` broke ld.so's object dedup (libc "was" the executable); `pslldq` was missing (ld.so's SSE2 strcmp); mmap read `fd=-1` as 64-bit and misclassified anonymous `MAP_FIXED` bss-zeroing as file-backed (stale `__exit_lock` → futex livelock). Debugged by an Opus subagent, verified independently. **Gateway to as-shipped distro binaries is open.**

## In flight

- Nothing active. Two backends agree on the corpus, the fuzzer, a vectorized SHA-256, a real musl libc program, a float program, self-modifying code, 8-way threading, a contended atomic counter, **real busybox** (`sha256sum`, `wc`), **real sqlite3** (in-memory query), **real lua** (x87), and **dynamically-linked binaries against both musl and glibc**. Live backlog: [`../tasks/open-backlog.md`](../tasks/open-backlog.md). **Next options:** **python** (the biggest static rung), an as-shipped distro binary through the glibc path, Jaguar-class ISA extensions (SSSE3/SSE4.x — `pshufb`, `palignr`, `pextrd`/`pinsrd`, `ptest`), true-80-bit x87, or the **M7 TSO barrier tiers** (needs an ARM host).

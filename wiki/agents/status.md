...
- **x87 FPU + real lua interpreter.** The x87 floating-point stack (fld/fild/fst/fstp/fistp incl. 80-bit fldt/fstpt, fadd/fsub/fmul/fdiv +p/r, fld1/fldz, fchs/fabs, fxch, fucomi/fcomi, fldcw/fnstcw/fnstsw, fprem) — **backed by `f64`, not the architectural 80-bit** (exact for double/float/int-origin values; lossy only in the extra long-double mantissa bits, so raw `%Lf` output can differ). One shared `exec_x87` drives both backends. An unmodified **lua 5.2** runs a table/arithmetic/string script three ways (native==interp==JIT): the VM math is SSE2 but musl's number parse/format needs x87. x87 vectors on exact values match interp==JIT==real 80-bit CPU.
...
- Nothing active. Two backends agree on the corpus, the fuzzer, a vectorized SHA-256, a real musl libc program, self-modifying code, 8-way threading, a contended atomic counter, and a full ladder of **real production programs**: busybox (`sha256sum`, `wc`), sqlite3, lua (x87), dynamically-linked musl + glibc hellos, and **CPython 3.13**. Live backlog: [`../tasks/open-backlog.md`](../tasks/open-backlog.md). **Next options:** an as-shipped distro binary through the glibc path, Jaguar-class ISA extensions (SSSE3/SSE4.x — `pshufb`, `palignr`, `pextrd`/`pinsrd`, `ptest`), true-80-bit x87, or the **M7 TSO barrier tiers** (needs an ARM host).

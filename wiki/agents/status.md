...
- Nothing active. Two backends agree on the corpus, the fuzzer, a vectorized SHA-256, a real musl libc program, a float program, self-modifying code, 8-way threading, a contended atomic counter, **real busybox** (`sha256sum`, `wc`), **real sqlite3** (in-memory query), and **real lua** (x87). Live backlog: [`../tasks/open-backlog.md`](../tasks/open-backlog.md). **Next options:** **dynamic linking** (`DYN-*`: ld.so, relocations, mmap/mprotect — gateway to most distro binaries), more static programs (python, file-DB sqlite → writable-file passthrough; raw float `%Lf` output needs true 80-bit x87), or the **M7 TSO barrier tiers** (needs an ARM host).

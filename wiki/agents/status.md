...
- **Dynamic linking (musl).** A real `ET_DYN` PIE musl binary runs three ways (native==interp==JIT): `load_dynamic_elf` maps the exe + `ld-musl` at load biases with a full auxv, and the real ld.so relocates it in-guest. **The core was untouched** — the feature is entirely the ELF loader (`x86jit-elf`) + the mmap(`MAP_FIXED`)/`mprotect` shim, confirming the guest/OS boundary (§1). glibc (runtime `.so` mmap of `libc.so.6`) is the remaining rung (`DYN-T5`).

## In flight

- Nothing active. Two backends agree on the corpus, the fuzzer, a vectorized SHA-256, a real musl libc program, a float program, self-modifying code, 8-way threading, a contended atomic counter, **real busybox** (`sha256sum`, `wc`), **real sqlite3** (in-memory query), **real lua** (x87), and a **dynamically-linked musl PIE**. Live backlog: [`../tasks/open-backlog.md`](../tasks/open-backlog.md). **Next options:** **glibc dynamic** (`DYN-T5`: runtime `.so` mmap → as-shipped distro binaries), more programs (python, file-DB sqlite), true-80-bit x87 (raw `%Lf`), or the **M7 TSO barrier tiers** (needs an ARM host).

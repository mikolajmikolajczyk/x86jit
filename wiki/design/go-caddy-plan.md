...
| **The mt substrate exists and is proven** — clone→host-thread recipe + condvar futex + `Arc<Vm>` run loop, but only inside a *test harness*, not the shim | `x86jit-tests/tests/mt.rs:38-78` (futex), `:152-185` (clone), `:91-121` (run loop); M7 T1–T4b done per `wiki/tasks/m7-multithreading-tso.md` |
...
committed") — record it as an ADR (`wiki/adr/`) since it forecloses per-page
...
- **Clock becomes real** in mt mode: the virtual tick-per-read clock
  ...
  (Deliberate determinism loss — record in `wiki/decisions/`.)
...
panic). Sketch it in `wiki/agents/deferred.md`; pull forward only on symptom
(a).

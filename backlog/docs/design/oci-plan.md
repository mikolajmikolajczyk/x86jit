---
id: doc-18
title: 'oci — phased implementation plan'
type: specification
...
- **Mechanism** — new `x86jit-tests/src/compat.rs` + bin `src/bin/compat.rs` (needs
  ...
  4. Generation map (checked-in constant): **v1** = base+SSE+SSE2(+CMOV, FXSR); **v2**
     ...
     string/atomics.
- **Artifacts** (generated, checked in): `backlog/docs/compat/coverage.json` (per-feature
  total/lifted/partial/missing code lists) + `backlog/docs/compat/isa-coverage.md` (per-gen

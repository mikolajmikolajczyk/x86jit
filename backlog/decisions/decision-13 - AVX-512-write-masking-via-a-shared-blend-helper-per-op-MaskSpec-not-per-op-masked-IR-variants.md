...
op — the risk (task-118) is that adding a masked IR variant per op doubles the
...
- **`CpuState::write_masked(dst, newval: [u128;4], k, elem, zeroing, bytes)`** — the
 ...
 commits via `set_vec` (task-118.3). Cranelift gets the mirror emitter.
...
- Compares-to-opmask (`vpcmp → k`, task-116.5) are unaffected: their write-mask

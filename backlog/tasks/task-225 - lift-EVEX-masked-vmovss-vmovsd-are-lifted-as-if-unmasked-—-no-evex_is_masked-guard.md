...
MEASURED at the then-current HEAD, GuestCpuFeatures::v4, k1 = 0 (all mask bits clear), so merge-masking must leave DEST completely untouched:
...
Minimum viable fix is to reject the masked forms in the lift (Exit::UnknownInstruction) as the sibling lifts do; implementing the mask is a separate step. Found while investigating task-223.

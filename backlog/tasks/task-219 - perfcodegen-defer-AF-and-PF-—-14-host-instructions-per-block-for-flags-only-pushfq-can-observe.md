...
Measured under task-216. Per-flag cost in a lifted block (density_tests, chained exit, 32-bit `add`):
...
MEASURED (density_tests, `alu reg,reg`, chain-fixed host instructions):
    ...
    baseline           56.0
    after TASK-218     50.7   -9.5%
    after TASK-219     40.3  -20.5%   (-28% cumulative)
...
NEXT: this and TASK-218 together are the cheap falsifiable probe for the direction. -28% of a
block's emitted code. If the embedder measures no fps change, the 'frontend-bound = hot code size'
model is wrong and TASK-220 (full lazy flags) must not be started.
<!-- SECTION:NOTES:END -->

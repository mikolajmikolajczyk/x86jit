...
This is not hypothetical — it is how TASK-289 survived. The regression test vmovs_vex_merge_takes_upper_from_vvvv (x86jit-tests/tests/jit.rs) PASSES under cargo test and FAILS under cargo test --release, on the same tree:
...
- [ ] #3 The known release-only failure (TASK-289) is caught by the new leg
<!-- AC:END -->

...
Found while investigating task-289 (which turned out to be a false report); this is a real, separate defect in the same silent-because-usually-zero class. Invisible to the whole corpus because jit_eq_interp starts from an all-zero CpuSnapshot and no test seeds zmm_hi before a VEX.128 op.

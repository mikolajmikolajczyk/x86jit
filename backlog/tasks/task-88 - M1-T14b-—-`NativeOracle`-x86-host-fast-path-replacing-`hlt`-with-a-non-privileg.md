...
<!-- SECTION:NOTES:BEGIN -->
Realized by task-130 (x86jit-tests/src/native.rs). The NativeOracle uses a fork + SIGSEGV-on-hlt capture (non-privileged terminator via a sigaltstack handler) instead of a hlt-replacement, per this task's intent. YMM/ZMM upper-half capture tracked in task-135.
<!-- SECTION:NOTES:END -->

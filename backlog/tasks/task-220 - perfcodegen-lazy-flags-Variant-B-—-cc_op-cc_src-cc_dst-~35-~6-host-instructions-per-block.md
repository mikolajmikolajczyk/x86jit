...
DO NOT START before TASK-219 has landed AND the embedder has re-measured. TASK-219 is the cheap falsifiable probe for this whole direction; if a measured ~20% cut in hot code does not move fps, this task is dead and must be closed as such.

Recorded in backlog/docs/deferred.md as 'Lazy flags (Variant B)', deferred at design time with 'revisit when M5, once the JIT works and profiling shows flag computation is hot'. task-216 is that profiling. TASK-85 delivered the compile-time substitute (dead-flag elimination); this is the runtime form the substitute was standing in for.

THE MEASUREMENT (task-216, density_tests with a chained exit):
...
- [ ] #2 TASK-219's result has been measured on the embedder and shows the frontend model transfers; otherwise this task is closed unimplemented with that result recorded
...
`density_tests`, `alu reg,reg`, host instructions in a block that exits by chaining:
    ...
    baseline (before TASK-218)   56.0
    after TASK-218               50.7   -9.5%   (constant-pool masks removed)
    after TASK-219               40.3  -20.5%   (AF/PF stored as sources)
...
FRONTEND-bound: task-217 removed ~38 million helper calls per second from it with
...
reading holding. It gets taken only if TASK-219 transfers to the embedder.
...
- **The oracle.** The interpreter validates the JIT. Keeping the interpreter eager
  ...
  TASK-219, and that is the mechanism this reuses.
...
- **The x86 quirks the FlagMask encodes** (inc/dec preserve CF; logic forces
  CF=OF=0; shift-by-0 preserves everything, see TASK-167; undefined flags must stay
...
exactly what TASK-219 is being measured for, and this plan does not start until
that answer is in.
<!-- SECTION:PLAN:END -->
...
The gate this task set on itself has fired NEGATIVE. bdb92b0 (task-218 + task-219) cut a block's
...
0% cold, and ZERO flag work — task-218/285 optimized flags, which SSE blocks do not emit. If the

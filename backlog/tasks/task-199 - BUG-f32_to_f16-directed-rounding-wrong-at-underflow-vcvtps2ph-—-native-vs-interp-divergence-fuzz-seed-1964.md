...
Found by the AVX fuzz driver (TASK-198, `fuzz_avx`). The interpreter diverges from the real host CPU (NativeOracle — ground truth for VEX) on `vcvtps2ph` when the imm8 selects a *directed* rounding mode (toward +inf / -inf) rather than round-to-nearest. The bug is in `f32_to_f16` (`x86jit-core/src/interp/mod.rs:2626`).

...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 two threaded guests sharing one blocking-mode host socket both blocked in read (or accept) with a single ready event do NOT deadlock; the loser re-parks and resumes on the next ready event
- [ ] #2 a regression test drives the two-reader-one-ready-event race and completes without hang
<!-- AC:END -->

...
FUZZ FINDING, PRE-EXISTING, NOW FILED AS TASK-211: the fuzzer reports a [JIT-vs-interp] divergence on vdpps (seed 26816) plus native-vs-interp on vdpps (seed 20980). Verified NOT caused by this change by stashing the whole task-210 diff and re-running seed 26816 on the pre-change tree — byte-identical divergence. Note the first comparison I ran was invalid (the fuzz binary uses JitBackend::new and does not read X86JIT_OPT_LEVEL, so the 'none' run was really Speed); the stash re-run is the one that counts.

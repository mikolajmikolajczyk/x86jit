...
<!-- SECTION:NOTES:BEGIN -->
DONE as the FOUNDATION (see task-111 for the real OSR). Delivered: the two-threshold plumbing — Vm::set_tier_up_region_after (T2), cache region_decision map (one lift_region per pc to decide loop candidacy), TierCfg::region_after, and the resolve split (a region candidate waits for T2, a non-loop tiers single-block at T1). Unit-tested: adaptive_tier_forms_a_region_only_for_a_long_loop (short loop <T2 -> no region, long loop >=T2 -> region, self-selected). Backward-compat: default T2=None uses T1 (BGT-6 unchanged); full suite green. SCOPE DISCOVERY: 'smart' switching that WINS on the corpus needs backedge counters IN compiled code (task-111) — a dispatcher-side counter can't promote a chained single-block loop (it never returns to the dispatcher), and interpret-until-T2 footguns a hot-but-short candidate (sha256 -> 0.1x at high T2). So T2 is NOT wired to the runner/bench yet (default path footgun-free); the corpus win lands with task-111.
<!-- SECTION:NOTES:END -->

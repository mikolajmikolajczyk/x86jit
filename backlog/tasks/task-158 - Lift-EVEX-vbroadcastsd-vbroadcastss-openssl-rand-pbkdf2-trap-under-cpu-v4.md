...
Found during task-36 (unemulinux) end-to-end: 'openssl rand -hex' and 'openssl enc -pbkdf2' trap UnknownInstruction under --cpu v4 on EVEX vbroadcastsd (62 f2 fd 28 5a ..., opcode 0x5A). openssl's v4 crypto/PRNG paths hit EVEX broadcast (5a=bcast sd, 18=bcast ss, plus vpbroadcast{b,w,d,q} 78/79/58/59). Lift the EVEX broadcast family (masked/zeroing, broadcast a scalar/element across the dest, reg+mem src). Established masked-EVEX helper->interp pattern + native bit-exact. Unblocks openssl rand + any v4 binary using broadcasts. Also note: AT_RANDOM real-bytes-in-HostEntropy (task-36 (unemulinux) leftover) needs a setup_stack API change - separate.

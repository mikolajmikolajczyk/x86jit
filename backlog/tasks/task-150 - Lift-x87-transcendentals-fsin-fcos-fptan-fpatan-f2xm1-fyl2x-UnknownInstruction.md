...
Discovered while deepening the x87 differential (task-132): the interpreter does NOT lift the x87 transcendentals — fsin/fcos/fptan/fpatan/f2xm1/fyl2x/fyl2xp1/fsincos all trap UnknownInstruction. Present in real binaries via libm's x87 fallback paths. Needs F80 (80-bit) implementations (f80.rs) matching x87 semantics + reduction; validate against a documented reference (Unicorn's QEMU x87 transcendentals are not Intel-exact, so use a bounded-ULP guard vs libm, per the task-132 harness). Lower priority than SIMD gaps but a real ISA hole. task-132 left a tripwire test that fails when these get implemented, prompting a real differential.

...
Measured under task-216. Disassembly of a lifted `add eax,ebx ; add eax,ebx ; jnz +2 ; hlt` block (density_tests::dump_one_shape, CHAIN=1) shows every flag computation masking the result to the operand size with a 64-bit `andq` against a CONSTANT-POOL entry rather than using the natural subregister.
...
PF and AF are now 15 of ~33 — TASK-219 takes them next.

...
- [ ] #4 conventions.md states the invariant and why a differential test cannot check it
- [ ] #5 No 256-bit handler writes its destination before both memory halves have loaded
- [ ] #6 Every faulting memory-source vector op carries its first source explicitly; VHFloatM is the documented pattern
- [ ] #7 The faulting sub-address (and for writes its width and value) reaches Exit::UnmappedMemory
- [ ] #8 Native fault-and-retry witnesses exist for the 256-bit dst==src1 case and the cross-page case

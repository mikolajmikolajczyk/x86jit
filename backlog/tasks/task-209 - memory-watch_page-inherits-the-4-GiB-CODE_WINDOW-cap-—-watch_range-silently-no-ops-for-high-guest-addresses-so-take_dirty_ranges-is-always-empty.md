...
- [x] #4 zero-cost-when-unwatched (task-148) is preserved: an unwatched memory still pays only the single relaxed watch_count load on the store path
...
Related: task-66 (unemulinux) (MADV_DONTNEED bypasses tracking) is NOT a factor for this embedder — its guest-facing madvise is a no-op stub, so guest DONTNEED never reaches the host and cannot zero watched pages behind the tracker's back.
<!-- SECTION:PLAN:END -->

...
Activate seams §17.3 + §17.4: `CpuMode` gains `Compat32`; the mode becomes a Vcpu/Vm construction parameter threaded through dispatcher -> cache -> `lift_block`/`lift_one` (today hardcoded `CpuMode::Long64` at lift.rs:71/132). Cache key becomes `BlockKey { guest_addr, mode }` (TASK-43 left the marker). `disasm.rs` takes the mode too. No 32-bit semantics yet — Long64 behavior must be bit-identical after the refactor.

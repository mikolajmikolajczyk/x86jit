...
IMPACT: latent — no real binary observed to trap here yet (compilers usually emit op2!=dst). But these are valid encodings (merge/replace-upper idioms, in-place shuffle) and silently produce wrong results. Discovered while fixing task-146.

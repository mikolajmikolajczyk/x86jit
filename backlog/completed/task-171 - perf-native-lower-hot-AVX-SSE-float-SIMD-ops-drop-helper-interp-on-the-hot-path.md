...
ps4-perf Tier-1, RE-SCOPED per the task-170 audit (doc-26). ORIGINAL premise (native-lower vmulps/vaddps/... for 2-10x) is void: the game-hot float core (add/sub/mul/div/min/max/sqrt/cmp, all packed-int, bitwise, imm-shifts, common shuffles/blends/broadcasts) is ALREADY native (builder.ins() -> NEON). No float-arith lever remains. Retargeted at the actually-helper-backed, PS4/Jaguar-reachable (SSE, 128-bit) ops from doc-26's ranked worklist: #2 shift_reg (psll/psrl/psra {w,d,q} xmm,xmm -- SSE2 scalar-xmm-count packed shift, currently helper 'shift_reg' cranelift/codegen/vector.rs) and #3 dpps/dppd (SSE4.1 dot product, currently helper 'dpps'). Native-lower both to Cranelift/NEON, bit-exact vs unicorn, measure on task-169's microbench. Expect single-digit % whole-program wins (not multiples) -- the hot float loops were never the helper cost. FMA (worklist #6) intentionally EXCLUDED -- big for AVX2+ guests but Jaguar/PS4 has no FMA3; belongs to a general (non-ps4-perf) track.

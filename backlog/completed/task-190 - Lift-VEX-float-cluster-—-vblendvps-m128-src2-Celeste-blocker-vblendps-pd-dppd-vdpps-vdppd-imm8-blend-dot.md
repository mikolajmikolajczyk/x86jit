...
Celeste (FNA/MonoGame, AVX-heavy) faulted on c4 e3 59 4a 1d ... = vblendvps xmm3, xmm4, [rip+disp32], xmm3 — the VEX variable-blend with an m128 second source, which lift_vblendv explicitly defers (mem src2 deferred). Lift that memory-source form (primary blocker), plus the cheap adjacent VEX/SSE imm8-blend + dot-product cluster whose SSE base is (or can be) a mechanical reuse: vblendps/vblendpd (+ SSE blendps/blendpd), dppd (SSE) + vdpps/vdppd (VEX 3-operand). Mirror the task-189 vinsertps pattern: aliasing-safe explicit-merge-base IR, VZeroUpper for VEX.128, vex_eq_sse + native_*_matches_interp differential tests, three tiers (decode/interp/cranelift).

...
Depends on TASK-201 (the op table lets the generator know a program is float-heavy and bias operands accordingly) though it can also land independently as a pure vec128 extension.
...
- [ ] #4 A targeted run (cargo xfuzz --ops vcvtps2ph, once TASK-199 is fixed) exercises the underflow/overflow/subnormal boundaries within seconds, demonstrated by the coverage table
<!-- AC:END -->

...
The differential fuzzer (task-129) found these SSE2 packed-integer ops are NOT lifted (interp returns UnknownInstruction, real hardware runs them): packsswb, packssdw (signed byte/word packs; only packuswb is lifted), pmuludq, pmaddwd, pmulhw, pmullw (packed multiplies), pavgb, pavgw (packed average), paddusb/paddusw/psubusb/psubusw (unsigned saturating add/sub), paddsb/paddsw/psubsb/psubsw (signed saturating add/sub). Lift them (interp + codegen; several map to Cranelift's saturating/avg/mul packed ops). Then add them back to the fuzzer's VBin op list (currently capped at the 29 lifted ops). Low-priority but real SSE2 coverage gaps that real programs (image/audio codecs) hit.
...
- [ ] #2 fuzzer V_BIN_OPS menu re-extended with the new ops (task-129 restriction lifted)

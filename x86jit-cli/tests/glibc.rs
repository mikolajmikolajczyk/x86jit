...
//! which glibc's PLT resolver uses to preserve XMM across symbol resolution. Pulled
//! digest-pinned from the registry (no tar).

mod common;
use common::{oci, Native, BUSYBOX_GLIBC};

#[test]
fn busybox_glibc_echo_runs_three_ways() {
    oci(BUSYBOX_GLIBC, "gl-echo")
...
fn busybox_glibc_sha256sum_reads_rootfs() {
    ...
    let Some(ran) = oci(BUSYBOX_GLIBC, "gl-sha")

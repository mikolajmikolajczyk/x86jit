...
/// reading them straight into `snap.st[i]` needs no top-of-stack rotation.
...
/// The Unicorn oracle in 32-bit protected/compat mode (`UC_MODE_32`) — the 32-bit
/// differential lane. Same execution shape as [`UnicornOracle`]; the mode is a
...
fn load_regs(uc: &mut Unicorn<()>, snap: &CpuSnapshot, entry: u64, bits32: bool) {
    ...
    // x87: seed the control word and the stack in architectural order.
...
fn store_regs(uc: &Unicorn<()>, rip_override: Option<u64>, bits32: bool) -> CpuSnapshot {
    ...
    CpuSnapshot {
        ...
        xmm,
        // This Unicorn build can't run AVX, so it never sets YMM upper halves; leave
        // them zero. AVX tests use the interpreter, not this oracle.
        ymm_hi: [0; VREGS],
        // Likewise no AVX-512 state; ZMM upper halves and opmasks stay zero.
        zmm_hi: [[0; 2]; VREGS],
        kmask: [0; 8],
        st,
        fpu_cw,
...
/// Read the x87 state from Unicorn: the stack in architectural order

...
/// interpreter. Unicorn's QEMU build mis-decodes VEX 3-operand forms
...
/// Validate a VEX.256 snippet against an SSE-per-half reference on the interpreter.
...
/// Non-temporal stores `movntdq`/`movntps`/`movntpd` (16-byte vector) and
...
/// 8-bit one-operand `mul r/m8` / `imul r/m8` validated against Unicorn.
...
/// 8-bit one-operand `div r/m8` / `idiv r/m8` (F6 /6,/7) validated against
...
/// carries. AH bits 1/3/5 are not flags: bit 1 reads back set, 3 and 5
...
/// `sahf` writes only CF/PF/AF/ZF/SF — OF must survive it. The `add` sets
...
fn rotate_through_carry_by_one_matches_unicorn() {
    ...
    // that trapped the netpoller. Test both CF-in states via stc/clc.
    diff(
        |a| {
...
fn div_by_constant_carry_fold_matches_unicorn() {
    ...
    // netpoller — here validated end to end against the Unicorn oracle.
    diff(
        |a| {
...
fn x87_fistp_honors_rounding_control_matches_unicorn() {
    ...
    // ties-to-even. 1.5 rounds differently under each mode, so a mode-ignoring
...
/// `fistp` of a value in [0.5, 1) previously panicked (`to_i64_rc` shift
...
fn x87_subnormal_fstp_tbyte_matches_unicorn() {
    ...
    // multi-bit subnormals, so the stored mantissa was off by up to 2x.
...
/// `fnstenv m28byte` — the 28-byte x87 environment image (SDM Vol 1 Figure
/// 8-9, the 32-bit protected-mode layout 64-bit mode uses at the default operand size).
///
/// Compared field-wise instead of through `diff()`, because three parts of the image are
...
/// * **bit 6 of the control word.** A third measured divergence.
...
/// `fnstenv` masks every FP exception after saving the environment (SDM:
...
/// `fldenv m28byte` honors the control word it loads.
///
...
/// x87 integer-operand arithmetic — `fiadd`/`fimul`/`fisub`/`fisubr`/
...
/// The integer-operand forms must produce *bit-identical* 80-bit results to
...
// ---- Deepened x87 differential (full stack + inexact + transcendentals) ----

/// Basic x87 arithmetic on operands whose true result is NOT representable in
/// 64 significand bits must match the real 80-bit FPU BIT-EXACTLY — no tolerance.
/// The full 80-bit ST(0) is left on the stack and compared against Unicorn's ST0, so a
/// wrong low mantissa bit cannot hide behind a truncating f64 read-back into a GPR.
#[test]
...
/// Run `build` on the interpreter and read ST(`i`) rounded to f64.
...
/// The x87 transcendentals are lifted at f64 precision. This checks that the INTERPRETER
/// executes each op with the correct stack effect and produces a result within a tight
/// ULP bound of the `f64` libm reference (the interp computes via libm, so this is ~0
/// ULP; the bound guards the lift/stack wiring — a wrong top-of-stack, pop count, or
/// push would blow it wide open).
#[test]
...
/// Harness guard on the x87-stack capture, exercised through the transcendentals.
///
...
/// Unicorn's ST(0) is meaningless as a hardware oracle. Instead of a false-precise bit
/// compare, this reads ST(0) back through the Unicorn oracle and asserts the captured
/// result rounds to within a documented, small ULP bound of the Rust `f64` libm
/// reference — a broken ST read-back, wrong top-of-stack mapping, or byte order would
/// blow the bound wide open, without pretending to hardware-exact transcendental
/// parity.
///
...
/// MMX↔XMM bridge `movq2dq`/`movdq2q` + `emms`. MMX aliases the low 64 bits of
...
/// Selecting `X87Precision::Extended` routes the x87 transcendentals through
...
/// tail as a bogus fault. A real regression: Go's bignum crypto (`p521Square`)
/// has >4 KiB branchless stretches. ~2600 two-byte
...
// --- AVX (VEX.128). Each VEX.128 form must lower to the same state as
...
/// `vextractps r/m32, xmm, imm8` (VEX.128.66.0F3A.W0 17) — extract the
...
/// `vextractps m32, xmm, imm8` — the memory-destination form (the exact
...
/// VEX.128 `vinsertps xmm1, xmm2, xmm3, imm8` (3-operand). Unicorn's QEMU build
...
/// The m32 form `vinsertps xmm1, xmm2, m32, imm8` — the inserted dword comes from
...
/// The exact wild encoding that walled a real guest — `c4 e3 79 21 d1 10` =
...
/// The exact encoding that walled a real guest's audio library — `c4 e2 3d 2e 11` =
...
// --- VEX float-op sweep — vsqrtp{s,d}, vrsqrtss/vrcpss (scalar, m32) +
// vrsqrtps/vrcpps (packed), vshufps/vshufpd, SSE float unpck bases + VEX vunpck*. ---

/// The exact wild encoding that walled a real guest — `c5 fa 52 d0` =
...
/// VEX packed sqrt `vsqrtps`/`vsqrtpd` (2-operand, no vvvv). Validated against the
...
/// VEX 3-operand shuffles `vshufps`/`vshufpd` — distinct merge base (vvvv). The SSE
...
fn vshuf_vex_eq_sse() {
    vex_eq_sse(
        ...
        |a| {
            ...
            // legacy m128 form (which has its own tests).
            a.mov(rax, SCRATCH).unwrap();
            a.movdqu(xmmword_ptr(rax), xmm1).unwrap();
...
/// SSE float unpacks `unpcklps`/`unpckhps`/`unpcklpd`/`unpckhpd` + their VEX
...
// --- VEX.256 (YMM) float sweep — vcvt{dq2ps,ps2dq,tps2dq}, vadd/sub/mul/div/
...
/// The exact wild encoding that walled a real guest — `c5 fc 5b c0` =
...
/// 256-bit lane-preserving converts `vcvtdq2ps`/`vcvtps2dq`/`vcvttps2dq ymm`.
...
/// 256-bit packed arithmetic `v{add,sub,mul,div,min,max}{ps,pd} ymm`. SSE-per-half
...
/// 256-bit packed `vsqrt{ps,pd} ymm`. SSE-per-half reference. Covers reg + m256.
#[test]
fn vsqrt_ymm_eq_sse() {
    vex256_eq_sse(
        |a| {
...
/// 256-bit `vshuf{ps,pd} ymm` (per-128-lane) + `vunpck{l,h}p{s,d} ymm`. SSE-per-half
...
// --- SSE-per-half reference builders for the VEX.256 differential tests. Each splits the two
...
// --- VEX float cluster — vblendv m128 src2 (a reported blocker) + imm8 static
...
/// The exact wild bytes that walled a real guest — `c4 e3 59 4a 1d e6 c7 07 00 30` =
...
/// SSE4.1 variable blend `vblendvps/pd`/`vpblendvb` with a 128-bit MEMORY src2.
...
/// SSE4.1 imm8 static blends `blendps`/`blendpd` (register + m128 src2), validated
...
/// AVX `vblendps`/`vblendpd` (VEX 3-operand imm8 static blend, register + m128
...
/// SSE4.1 `dppd` double-precision dot product (register + m128 src2), validated
...
/// AVX `vdpps`/`vdppd` (VEX 3-operand dot product, register + m128 src2), validated
...
/// VEX.128 `vmovlhps`/`vmovhlps` (3-operand) lower to a 64-bit-lane unpack.
...
/// The exact wild shape `vmovlhps %xmm0,%xmm1,%xmm0` (dst == op2). `VUnpackLow`
...
/// SSE3 lane-duplicating moves `movddup`/`movsldup`/`movshdup`, register and
...
/// VEX.128 `vmovddup`/`vmovsldup`/`vmovshdup` lower to the same shuffle as the
...
fn fwait_is_a_noop() {
    ...
    // exceptions it must not perturb any architectural state.
    diff(
        |a| {
            a.mov(rax, 0x1234_5678_9abc_def0u64).unwrap();
            a.wait().unwrap(); // 0x9B
...
fn vzeroupper_clears_all_upper() {
    ...
    // VZEROUPPER). Seeding registers 16-31 and asserting they SURVIVE is what makes
    // this a test of the register range rather than of the clearing alone.
    assert!(
...
/// Packed float↔int converts: `cvtdq2ps/cvtps2dq/cvttps2dq/cvtdq2pd/cvtps2pd/
...
/// vcvtpd2dq/vcvttpd2dq/vcvtdq2pd`) match their legacy-SSE equivalents on the
...
/// Register-count packed shifts `psll/psrl/psra {w,d,q} xmm, xmm`: the count is the
/// full low qword of the second operand. Covers logical L/R, arithmetic
...
/// Upper-bits (255:128) semantics for register-count shifts: legacy-SSE
...
/// MOVMSKPS / MOVMSKPD: pack the packed-float sign bits into a GPR. Regression
...
// --- SSE4.1 / AVX ROUND family. Legacy `round{ss,sd,ps,pd}` are diffed
...
// --- Integer unpack / pack with a 128-bit MEMORY source. The register forms
...
// --- Non-temporal moves. The cache-bypass hint is a no-op in our coherent
...
// --- SSSE3 packed-integer horizontal add/sub: ph{add,sub}{w,d,sw}.
...
// For each 64-bit half: sum(|a.byte[i] - b.byte[i]|) over the 8 unsigned

...
/// As [`jit_eq_interp`] but with an explicit guest CPU feature set, so an AVX-512
/// snippet can run under `GuestCpuFeatures::v4()`.
...
/// Go's runtime memmove emits it — a real caddy binary trapped here.
#[test]
...
fn fwait_is_a_noop() {
    ...
    // state as the interpreter with no codegen for it.
    jit_eq_interp(
        |a| {
...
/// The JIT assembles the flag byte from its own materialized flag state — including PF
/// and AF, which are stored as SOURCES and so have to be derived on this path — while
/// the interpreter goes through `Flags::to_flags16`. Two independent constructions of
/// the same byte is exactly what this comparison is for.
#[test]
...
/// `SAL` is the `/6` encoding alias of `SHL` (identical semantics). iced's
...
/// `bsf`/`bsr` with a zero source set ZF=1 and leave the destination as its
...
/// A 66h `push imm16` transfers 2 bytes (not the 8-byte long-mode default),
...
/// A variable-count shift/rotate with a runtime count of 0 must leave EVERY
...
/// A flag *read* AFTER a possibly-no-op variable-count shift. Here
...
/// Non-temporal stores lower to plain stores — `movntdq`/`movntps`/`movntpd`
...
/// 8-bit one-operand `mul r/m8` / `imul r/m8` (F6 /4,/5) — AL*src8 → AX (AH:AL), CF/OF
/// from a non-zero AH. Covers no-overflow, overflow, and signed cases. The fuzzer found
/// this form unlifted; this pins the AH:AL + flag semantics.
#[test]
...
/// 8-bit one-operand `div r/m8` / `idiv r/m8` (F6 /6,/7) — AX / src8, quotient
...
/// 8-bit `div r/m8` by zero raises #DE (vector 0), like the wider forms.
#[test]
...
/// 8-bit `div r/m8` quotient overflow raises #DE. AX=0x2000 / 1 = 0x2000, which
...
/// 8-bit `idiv r/m8` quotient overflow raises #DE via the SIGNED bound (a
...
// The in-span-but-unmapped interp/JIT oracle gap is closed for every host-backed span
// by guard pages: the runner's non-Go Flat and Go Reserved paths both fault
// `UnmappedMemory` under the JIT now, pinned in
...
fn unknown_instruction_reports_real_bytes() {
    ...
    // error, not 15 zeros — so compat triage isn't misdirected. `ptest`, `pcmpistri`,
    // `dpps`, `mpsadbw` and `pmaddubsw` used to sit here but are now lifted, as are the
    // float `vmaskmovps/pd`; the integer `vpmaskmovd/q` are not.
    let mut asm = CodeAssembler::new(64).unwrap();
...
fn run_compiled_decodes_exception_not_panic() {
    ...
    // `Exit::Exception`, not fall through to its `panic!`. Materialize a #DE
...
/// `lock bts/btr/btc [mem], reg|imm`: the locked memory bit-ops lift to an atomic RMW.
/// Single-threaded this can't observe the atomicity, but it pins that
...
/// `fnstenv m28byte` writes 28 bytes of guest memory through the JIT's raw
...
/// `fldenv m28byte` reads 28 bytes of guest memory through the JIT's raw
...
/// x87 integer-operand arithmetic — all twelve encodings (`DA /n` = m32int,
...
/// x87 `fisttp` (SSE3): store ST(0) as an integer truncating toward zero (unlike
...
/// VEX-128 `vpmov{z,s}x*`: zero/sign-extend narrow → wide lanes with VEX's
...
/// 256-bit VEX float sweep — vcvt{dq2ps,ps2dq,tps2dq}, vadd/sub/mul/div/min/max
...
/// AVX1 `vmaskmovps`/`vmaskmovpd` — vector-mask conditional load/store, xmm+ymm,
...
/// AVX-256 data movement: the JIT must handle 256-bit `vmovdqu`
...
/// AVX-256 logic / packed arithmetic / movemask: register and 32-byte
...
/// AVX2 broadcast / 128-lane insert+extract: register and memory-source
...
/// AVX2 256-bit vpshufb (per-lane) + VEX shift-by-immediate, 256 and 128.
#[test]
...
/// AVX2 cross-lane permutes: vpermq (imm), vpermd (reg control),
...
/// CPU feature selection: the guest's `cpuid`/`xgetbv` observe the
...
/// — JIT == interp; both flagless, so flags stay put.
#[test]
...
/// BMI2 flagless shifts/rotate: shlx/shrx/sarx/rorx reuse the existing
...
/// BMI1/BMI2 family: andn/blsi/blsr/blsmsk/bextr/bzhi — the JIT's bmi
...
/// v3 scalars tzcnt/lzcnt/movbe: counts defined on a zero source (=
...
/// Host codegen target: a JIT pinned to `HostTarget::Baseline` (no AVX)
...
/// AVX-512 write-masking: masked `vmovdqu32 xmm{k1}` merge + `{k1}{z}`
...
/// AVX-512 foundation: unmasked 512-bit `vmovdqu64` load, `vmovdqa64`
...
/// AVX-512 grind: EVEX/VEX scalar-ish ops that CachyOS v4 binaries
...
/// AVX-512 opmask moves: kmov{b,w,d,q} between GPR, opmask, opmask,
...
/// AVX-512 opmask subsystem: mask-producing compare `vpcmpb` → k and
...
/// Memory-source `src2` for the EVEX mask-producing compares: glibc folds the second
/// operand as a load (`vpcmpeqb k, zmm, [rsi]`). The B operand is staged into
...
/// Memory-source `src2`/`src3` for the unmasked EVEX data ops, at 512-bit:
...
/// AVX-512 write-masked **memory** moves: `vmovdqu8 v{k}{z}, [mem]` (load,
...
/// AVX-512 ops the real v4 coreutils corpus hits: per-lane population count
...
/// VEX/EVEX scalar float arithmetic + int conversions the coreutils corpus hits:
...
/// AVX-512 `vptestm`/`vptestnm` → opmask: `k[i] = (a & b) != 0` (or `== 0`
...
/// AVX-512 dedicated-opcode masked compares: the EVEX forms of
...
/// AVX-512 EVEX lane ops: `vinserti32x4`/`64x2` (128-bit lane insert),
/// `vinserti64x4` (256-bit half insert) and `valignd`/`valignq` (cross-512 element
/// shift), each crossing a lane boundary — JIT == interp.
#[test]
...
/// AVX-512 masked EVEX logic: `vpxor/vpand/vpor{d,q}` with a write-mask,
...
/// MMX↔XMM bridge `movq2dq` / `movdq2q`: XMM→MMX→XMM round-trip through the
...
/// Masked EVEX unary lane ops `vplzcnt{d,q}` / `vprol{d,q}` / `vpconflict{d,q}`,
...
/// Masked EVEX blend `vpblendm{d,q}`, merge + zeroing at 128/256-bit. JIT must
...
/// Masked EVEX 128-bit-lane shuffle `vshuff32x4` / `vshuff64x2` at 256-bit,
...
/// Masked EVEX `vpmultishiftqb` at 128-bit, unmasked + masked merge + zeroing.
...
/// FMA3 `vf[n]m{add,sub}{132,213,231}{ss,sd,ps,pd}`: fused multiply-add across
...
/// FMA alternating-sign `vfmaddsub`/`vfmsubadd{132,213,231}{ps,pd}`, xmm AND ymm,
...
/// VEX.256 float horizontal to ymm — `vh{add,sub}p{s,d}` / `vaddsubp{s,d}` in the
...
/// EVEX lane broadcast `vbroadcast{i,f}{32x4,64x2,32x8,64x4}`, memory chunk,
/// unmasked + masked merge + zeroing. JIT must match interp bit-for-bit (native oracle
...
/// (merge + zeroing) at 128/256-bit + a masked memory operand. JIT must
...
/// Dword packed min/max `vpmin/max{u,s}d` (VEX.128 + EVEX-512): perl/python3
...
/// Cross-lane permutes: index-mode `vpermi2d`, single-source vector-index
...
/// VEX-128 grab-bag the python3 SIMD paths hit: 3-operand `vinserti128` from
...
/// `vpblendw` VEX.128 with an m128 src2 (the reported form
...
/// `vpshufd` on ymm/zmm (python3): per-128-bit-lane dword shuffle, unmasked +
/// masked. JIT == interp.
#[test]
...
/// store `vpmovqd [mem]←xmm`. The store result is reloaded into a register so
...
/// `vpabs{b,d,q}`: openssl/curl hit vpmullq, vim hits vpabsb. JIT == interp.
#[test]
...
/// EVEX-512 `vpshufb zmm` per-128-bit-lane byte shuffle (cal), unmasked +
...
/// Opmask shift `kshift{l,r}{b,w,d,q}` (vim): shift by imm8 within the width,
...
/// Opmask bitwise logic family `k{or,and,andn,xor,xnor}{b,d}` + `knot`: glibc's
...
/// EVEX narrowing (truncating) move `vpmov{dw,qd,wb}`, unmasked + merge/zero
...
/// compute per-lane then merge/zero-mask. Covers partial, all-ones and
...
/// EVEX scalar `vrndscale{sd,ss}` with scale factor M=0: a 3-operand
...
/// VEX.128 helpers the coreutils corpus hits: `vpunpcklqdq` interleave,
...
/// 3-operand VEX scalar/packed float ops where op2 aliases the destination —
...
/// The rest of the VEX 3-operand `op2==dst` aliasing family (siblings of the `vaddsd`
/// case above) — in-place ops that previously pre-copied op1 into dst and so clobbered a
/// register op2 aliasing dst: `vpshufb`, `vpalignr`, `vroundsd`, `vsqrtsd`, `vmovsd`.
/// Each now carries an explicit source in its IR op. Register op2 == dst below; native
/// cross-check (`native_vex_alias_family_*`) validates the semantics against the CPU.
#[test]
...
/// Memory-source `pcmpistri`: `pcmpistri xmm, [mem], imm` — the loaded 128-bit
...
/// AVX-512 512-bit logic observable via the ZMM snapshot: `vpxorq`/
...
/// SSE4.2 `pcmpistri`/`pcmpestri`: the string-aggregation index (ECX) and
...
/// SSE4.2 `pcmpistrm`/`pcmpestrm`: the string-aggregation result written as a
...
/// SSE4.1 `insertps`: insert a src dword into a dst lane and zero lanes via
...
/// SSE4.1 `dpps`: single-precision dot product with a partial product mask and a
...
fn sse41_dpps_match_interp() {
    ...
    jit_eq_interp(
        |a| {
            ...
            // imm=0xFF: every product summed, broadcast to every lane.
...
/// SSE4.1 variable blend + `round`: `blendvps/blendvpd/pblendvb` select
...
/// SSE4.1 dword min/max: `pmin/pmax s/u d` reuse the existing packed
...
/// SSE4.1 `pmovzx`/`pmovsx` (register + memory source) and `pmulld`:
...
/// AVX-512 EVEX bitwise logic + `vpternlog`: `vpxorq/vpandq/vpord/
...
/// snapshot compares directly — JIT == interp.
#[test]
...
/// AVX `vptest`: the flags-only AND test Go's AVX2 memory routines
...
/// AES-NI SSE + VEX: every op (aesenc/dec/enclast/declast/imc/keygen) plus a
...
/// SHA-NI SSE: every op (sha256rnds2/msg1/msg2, sha1rnds4/nexte/msg1/msg2),
...
/// SSSE3 `psign{b,w,d}` + VEX.128 `vpsign{b,w,d}`: per-element negate/zero/keep
...
/// GFNI `gf2p8mulb/gf2p8affineqb/gf2p8affineinvqb` (SSE) + VEX.128 `vgf2p8*`,
...
/// PCLMULQDQ `pclmulqdq` (SSE) + VEX.128 `vpclmulqdq`, register + memory second
...
/// EVEX-512 packed shift-by-imm at ZMM width, unmasked + merge/zeroing masked
...
/// `pmuludq`/`vpmuludq` unsigned low-dword → 64-bit product, SSE + VEX.128/256,
...
/// Memory-source single-table permute `vperm{q,d} v, idx, [mem]` (EVEX-512,
...
/// `vpblendd` per-dword immediate blend, VEX.128 + VEX.256. JIT (byte shuffle)
/// must match interp.
#[test]
...
/// `vzeroall` zeros the whole vector file (low 128 + uppers); `vzeroupper`
...
/// `vzeroupper` under AVX-512 must zero bits 511:128 of ZMM0–15 — including
...
/// The *implicit* upper-zeroing a VEX.128 write performs — lowered as a trailing
/// `IrOp::VZeroUpper` on one register, not the `vzeroupper` instruction.
///
/// The `VZeroUpperAll` fix above covers `vzeroupper`/`vzeroall`; the single-register op
/// kept clearing `ymm_hi` only, so bits 511:256 of a previously-dirtied ZMM stayed live
/// under the JIT and were zeroed under the interpreter and on hardware. Nothing caught
/// it because reaching it needs a dirty `zmm_hi` *before* a VEX.128 write, which the
/// corpus never set up.
#[test]
...
/// `pinsrw`/`vpinsrw` insert a 16-bit word into an xmm lane. A prior JIT bug
...
/// `vextractps r/m32, xmm, imm8` (VEX.128.66.0F3A.W0 17) extracts the
...
/// `vpermilps`/`vpermilpd` with imm8, VEX.128 — reg and memory source. Both
...
/// 16-bit `movbe` (byte-swap move) memory store/load. `movbe` reuses the
...
/// `vpermq`/`vpermpd` imm8 with a MEMORY source (VEX.256) — the mem form
...
/// Packed integer multiplies the openssl TLS handshake needs —
...
/// openssl TLS: per-element variable shifts `vp{sll,srl,sra}v{w,d,q}` and the
...
/// openssl TLS: `vpsrlq zmm,[mem],imm` — EVEX imm-shift with a memory source.
#[test]
...
/// openssl TLS: GFNI wide/masked `vgf2p8affineqb`/`vgf2p8mulb` on ymm/zmm, register
...
/// caddy HTTPS: SSE4.1 `pblendw` — imm8 word blend, dst is also src1.
#[test]
...
/// openssl TLS: VEX 4-operand variable blends and EVEX qword compare→mask.
#[test]
...
/// openssl TLS: `vextracti32x4 [mem],zmm,imm` / `vextracti64x4 [mem],zmm,imm` —
...
/// A boundary-straddling `vextracti64x4 [mem],zmm,imm` (two 128-bit lanes)
...
///
/// The reported address moved deliberately, and both tiers moved together.
...
/// Runs both backends manually because the `VectorInput` harness always page-rounds the
/// buffer.
#[test]
...
/// `pop [mem]` is restartable — if the destination store faults, RSP must be
...
/// SSE2 packed saturating add/sub (byte + word). Values chosen to hit both
...
/// Packed unsigned rounding average `pavgb`/`pavgw` — verifies the `+1`
...
/// Signed packs `packsswb` (word->byte) and `packssdw` (dword->word).
...
/// `pmaddwd` — pairwise signed 16x16 multiply, adjacent products summed into
...
/// Packed float↔int converts: JIT == interpreter bit-for-bit, including the
...
/// (the native JIT path). Same coverage as the differential test — logical L/R, arith R,
...
/// Upper-bits (bits 255:128) handling for register-count shifts: the legacy-SSE
...
/// MOVMSKPS / MOVMSKPD: JIT == interpreter for the sign-mask extraction,
...
/// Integer unpack/pack with a 128-bit memory source 2 — the JIT `_m` emit paths
...
/// SSE3 lane-combining packed float `h{add,sub}p`/`addsubp` — the JIT paths
...
/// Non-temporal moves lower like the aligned movdqa/movaps path — the JIT must
...
/// SSSE3 packed-integer horizontal `ph{add,sub}{w,d,sw}` — the JIT paths
...
/// One VEX.128 form from each of the round / unpack / addsub / non-temporal / horizontal
/// families, with the destination's upper 128 bits (`ymm_hi`) pre-dirtied so `VZeroUpper` is
...
fn vex128_new_ops_zero_ymm_upper_jit_eq_interp() {
    jit_eq_interp(
        |a| {
            ...
            a.vroundpd(xmm4, xmm0, 1u32).unwrap(); // floor
            a.vpunpckldq(xmm5, xmm0, xmmword_ptr(rax)).unwrap(); // mem source
            a.vaddsubpd(xmm6, xmm0, xmm1).unwrap();
            a.vmovntdqa(xmm7, xmmword_ptr(rax)).unwrap();
            a.vphaddw(xmm8, xmm0, xmm1).unwrap();
            a.hlt().unwrap();
        },
        |s| {
            s.xmm[0] = 0x4004_0000_0000_0000_3FF8_0000_0000_0000; // [1.5, 2.5]
...
/// SSE3 duplicating moves + their VEX.128 forms — JIT must match interp across
...
/// VEX.128 `vmovlhps`/`vmovhlps` — JIT must match interp, including the wild
...
/// VEX.128 `vinsertps` (3-operand) — JIT must match interp, including the m32
...
/// VEX variable blend `vblendvps/pd`/`vpblendvb` with an m128 src2 (a real-guest wall)
/// — JIT must match interp, including the `dst == mask` and `dst == src1` aliases (both
...
/// imm8 static blends `blendps`/`blendpd` (SSE, dst==src1) and `vblendps`/
...
/// Dot products `dppd` (SSE) and `vdpps`/`vdppd` (VEX 3-operand) — JIT == interp,
...
/// VEX packed sqrt `vsqrtps`/`vsqrtpd` (2-operand) + m128 source — JIT must match
...
/// VEX 3-operand shuffles `vshufps`/`vshufpd` — JIT must match interp, incl. the m128
...
/// Legacy SSE `shufps`/`shufpd xmm, m128, imm8` (`0f c6 08 1b` / `66 0f c6 10 01`,
...
///   shared-with-VEX IR op would destroy.
#[test]
...
/// SSE + VEX float unpacks (`unpck*ps/pd`, `vunpck*`) — JIT must match interp, incl.
...
/// Reciprocal / reciprocal-sqrt `vrcpss`/`vrsqrtss` (scalar, m32) + `vrcpps`/
...
// ---- Register-survival regression ----
//
// The SIMD lifts below (round, unpack/pack, horizontal float/int, addsub, psadbw,
// non-temporal moves) were only covered by hand-written jit.rs cases that set a
// handful of registers, so a lowering that clobbered an *unrelated* live register
// would leave both the interp and the JIT at the default 0 for that register and
// slip through `compare`. These tests pre-load a FULL sentinel register file — every
// GPR (bar the two the snippet needs for addressing/stack), all 16 XMMs, and every
// ymm_hi — before each op, so any register the JIT writes but the interpreter
// doesn't shows up as a divergence.
...
/// VEX packed-int sweep: saturating add/sub, rounding average, byte/word
...
// --- VEX blends + permute/shuffle/byte-shift/movdup widened to ymm. ---
...
const LANE_ASC_LO: u128 = 0x0F0E0D0C_0B0A0908_07060504_03020100;
const LANE_ASC_HI: u128 = 0x1F1E1D1C_1B1A1918_17161514_13121110;
const LANE_PAT_A: u128 = 0xA0A1A2A3_A4A5A6A7_A8A9AAAB_ACADAEAF;
const LANE_PAT_B: u128 = 0xB0B1B2B3_B4B5B6B7_B8B9BABB_BCBDBEBF;
...
fn avx2_vblend_imm_ymm_match_interp() {
    jit_eq_interp_features(
        ...
        |c| {
            c.xmm[0] = LANE_ASC_LO;
            c.ymm_hi[0] = LANE_ASC_HI;
            c.xmm[1] = LANE_PAT_A;
            c.ymm_hi[1] = LANE_PAT_B;
            c.xmm[2] = LANE_PAT_A;
            c.xmm[3] = LANE_PAT_B;
            for r in [4, 5, 6] {
...
// ---- VEX v3 converts + movmsk/test/round/dpps ymm + horizontal/sign ymm ----

const VEX256_A_LO: u128 = 0x0F0E_0D0C_0B0A_0908_0706_0504_0302_0100;
const VEX256_A_HI: u128 = 0xFF00_FF00_1234_5678_9ABC_DEF0_0011_2233;
const VEX256_B_LO: u128 = 0x8081_8283_8485_8687_0102_0304_0506_0708;
const VEX256_B_HI: u128 = 0x1112_1314_1516_1718_8898_A8B8_C8D8_E8F8;

fn vex256_init(c: &mut CpuSnapshot) {
    c.xmm[0] = VEX256_A_LO;
    c.ymm_hi[0] = VEX256_A_HI;
    c.xmm[1] = VEX256_B_LO;
    c.ymm_hi[1] = VEX256_B_HI;
    c.xmm[2] = VEX256_A_HI;
    c.ymm_hi[2] = VEX256_A_LO;
}

/// VEX.256 horizontal integer + sign ops widened per 128-bit lane.
#[test]
...
/// VEX.256 vmovmskps/pd (8/4-bit mask) + vtestps/pd flags (xmm and ymm).
...
/// VEX.256 vroundps/pd and vdpps (xmm + ymm), per-128-bit lane.
#[test]
...
/// VEX.256 width-changing float converts.
#[test]
...
/// SSE4.1/VEX specialists: phminposuw, mpsadbw (xmm + ymm).
#[test]
...
/// REX.W (64-bit) explicit-length string compares pcmpestri64/pcmpestrm64.
#[test]
...
/// VEX `vextract{f,i}128 [mem], ymm, imm8` — the memory-destination
...
fn vextract128_mem_dst_match_interp() {
    ...
    //   vextractf128 $1, %ymm1, -0x50(%rdx) -> c4 e3 7d 19 4a b0 01   <- the faulting bytes
    ...
    let build = |a: &mut CodeAssembler| {
        ...
        // negative displacement mirrors the real guest's shape (`-0x50(%rdx)`).
...
/// Two forms that used to be lifted as something they are not.
///
...
fn wrong_lifts_trap_rather_than_computing_something_else() {
    ...
    // `vpblendw dst, dst, [mem], imm8`. The memory form loads the operand

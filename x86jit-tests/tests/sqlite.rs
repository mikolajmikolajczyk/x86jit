...
fn sqlite_memory_query_native_interp_jit_agree() {
    let native = std::process::Command::new(concat!(env!("CARGO_MANIFEST_DIR"), "/programs/sqlite3.elf"))
        .args([":memory:", SQL])
        .output()
        .expect("run native sqlite3")
        .stdout;
    assert_eq!(native, b"385|10|100\n", "native sum/count/max of squares 1..10");
    ...
    assert_eq!(interp, native, "interpreter result != native");
    assert_eq!(jit, native, "JIT result != native");
}

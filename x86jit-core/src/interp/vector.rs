...
/// AVX `vblendv{ps,pd}`/`vpblendvb` with an m128/m256 src2. Read `a` (src1) and the `mask`
/// register before writing `dst` so either aliasing `dst` is safe; a fault on the load traps.
/// Each 128-bit lane blends independently; `set_vec` clears bits above `bytes`.
#[allow(clippy::too_many_arguments)]
...
/// SSE/AVX imm8 static blend `blendps`/`blendpd`/`vblend*` register src2. Read
...
/// As [`exec_v_blend_i`] but src2 is an m128/m256 memory operand. `a` is read
...
pub(crate) fn exec_v_p_round_m(
    ...
    // Fault atomicity (§16) — see `exec_v_logic256_m`. Every lane loads into
    // `out` first; nothing reaches the destination until the last load has succeeded.
    // `d` is read from `dst` (this is the two-operand SSE shape), so committing lane 0
    // early would also change what lane 1's own merge base would be on a retry.
...
pub(crate) fn exec_v_extract_lane_wide_m(
    ...
    let n = *num_lanes as usize;
    let base = *idx as usize * n;
    // Atomic whole-region semantics, matching the JIT. A real CPU faults the
    ...
    // The probe asks `Memory::probe_write`, which consumes NOTHING. A `vload` probe looks
    // equivalent — a load shares the region and `Trap` checks with a store — but it takes
    // from `pending_mmio`, while the embedder answers a `MmioWrite` through
    // `pending_mmio_write`; the retry then re-probes and the access loops forever.
    //
    ...
    // whereas aborting would reproduce that same loop.
    for i in 0..n {
        let ea = a.wrapping_add(i as u64 * 16);
...
/// Store the `pcmpstr` mask + flags. Shared by the register and memory arms.
...
/// SSE4.1 `dppd`: double-precision dot product. `dst` is also source 1.
#[allow(clippy::too_many_arguments)]
...
/// SSE4.1 `dppd xmm, m128, imm8`: source 2 is loaded from memory; a fault traps.
#[allow(clippy::too_many_arguments)]
...
/// AVX `vdpps`/`vdppd`: the VEX 3-operand dot product with a distinct merge base
...
/// As [`exec_v_dp3`] but src2 is a 128-bit memory operand. `a` is read before
...
pub(crate) fn exec_v_logic256_m(
    ...
    // Fault atomicity (§16): both halves compute into locals and commit
...
pub(crate) fn exec_v_packed_bin256_m(
    ...
    let av = read_val(*addr, &*temps);
    let (alo, ahi) = (cpu.xmm[*a as usize], cpu.ymm_hi[*a as usize]);
    // Fault atomicity (§16) — see `exec_v_logic256_m`.
    let rlo = match vload(cpu, mem, cur_addr, av, 16) {
...
/// VEX `vpmaddwd`/`vpmaddubsw` memory form: `dst = madd(a, [addr])`, each
...
pub(crate) fn exec_v_byte_shift(
    ...
    // Per-128-bit-lane byte shift: each half shifts independently, NOT a full
...
pub(crate) fn exec_v_fma(
    ...
    // Masked EVEX packed FMA: merge/zero the masked-off lanes at `prec`
...
pub(crate) fn exec_v_pack_wide_m(
    ...
    let av = read_val(*addr, &*temps);
    // Fault atomicity (§16): op1 comes from `a`, not from a lift-emitted copy
...
/// movmskps/movmskpd: the sign bit of each packed-float lane of `src` → the low
...
/// AVX `vpermilps`/`vpermilpd` variable (register) control: an IN-LANE permute
...
// --- AES-NI. Register + memory forms; shared pure-Rust primitives. ---
...
// --- SHA-NI. Register + memory forms; shared pure-Rust primitives.
...
// --- GFNI. Register + memory forms; shared pure-Rust primitives. ---
...
// --- PCLMULQDQ. Register + memory forms; shared `pclmul` primitive. ---
...
// --- SSSE3 psign. Per `lane`-byte element (1/2/4):
...
pub(crate) fn exec_v_shuffle16(
    ...
    // EACH 128-bit lane independently — NOT cross-lane.
    let av = cpu.vec_lanes(*a as usize);
    let mut r = [0u128; 4];
    for c in 0..(*bytes as usize / 16) {
...
pub(crate) fn exec_v_unpack_low_m(
    ...
    let av = read_val(*addr, &*temps);
    // Fault atomicity (§16) — op1 read from `a`, destination untouched until
    // the load succeeds.
    let av1 = cpu.xmm[*a as usize];
    match vload(cpu, mem, cur_addr, av, 16) {
...
pub(crate) fn exec_v_float_mov(
    ...
    // Byte-wise, NOT `(a & !m) | (src & m)`: LLVM's x86-64 backend miscompiles that shape,
    // losing bits 127:64 of the result, so the merge silently keeps the destination's own
    // upper bytes. It needs an i128 mask from a `select` of two constants — which
    // `lane_mask(prec.bytes())` is — AND the other operand loaded in a predecessor block.
    // Reproducer, evidence and the check for "is the toolchain fixed yet" live in
    // `backlog/docs/llvm-i128-miscompile/` (run.sh). Do not restore the masked form until
    // that script exits 1.
...
// --- 256-bit VEX packed float: each 128-bit half handled independently by the
...
pub(crate) fn exec_v_float_bin256_m(
    ...
    let base = read_val(*addr, &*temps);
    let (alo, ahi) = (cpu.xmm[*a as usize], cpu.ymm_hi[*a as usize]);
    // Fault atomicity (§16) — see `exec_v_logic256_m`. Witnessed by
...
pub(crate) fn exec_v_h_int_m(
    ...
    let av = read_val(*addr, &*temps);
    // Per-128-bit lane: op1 comes from `a`, not from a lift-emitted pre-copy into `dst`,
    // so the destination is untouched if a load faults (fault atomicity, §16).
    // Load each lane's op2 and combine (phadd/phsub pair adjacent lanes within a 128-bit
    // lane; psadbw is per-64-bit). Results land in `out` and commit together.
    let lanes = (*bytes as usize) / 16;
    let mut out = [0u128; 2];
    for (l, o) in out.iter_mut().enumerate().take(lanes) {
        let ea = av.wrapping_add((l * 16) as u64);
...
/// rather than the way a Rust `as` cast happens to. Ordinary values are
...
/// Packed float↔int convert `cvt*p*`. Per-lane Rust `as` casts (saturating,
...
/// One 128-bit `cvt*p*` lane group: convert the packed lanes of `s` per
...
/// VEX.256 width-changing packed convert. Widening (`dq2pd`/`ps2pd`): read the

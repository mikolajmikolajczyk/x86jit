...
- **x87 exception reporting is partial.** The status-word flags (IE/ZE/OE/UE/PE) are set by arithmetic and validated against a real CPU, and an unmasked exception is delivered as `#MF` on the following instruction. Still missing: the stack-fault flag and C1, the condition codes C0/C2/C3 (which is why `ficom`/`ficomp` stay unlifted), and DE for a denormal operand (`TASK-328`). MXCSR is untouched on the SSE side — stored, reloaded, and governing nothing.

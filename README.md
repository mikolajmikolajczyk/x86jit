...
- **A vector access to an MMIO (`Trap`) region cannot be completed.** The embedder is asked for a value and the same exit comes back forever, because a 16-byte access is two transfers and the answer channel carries one (`TASK-332`). Scalar MMIO resumes correctly.
...
- **Translation-cache invalidation is not race-free** against another vcpu publishing a chained link or an indirect-branch-cache entry (`TASK-323`). Single-vcpu execution is unaffected.

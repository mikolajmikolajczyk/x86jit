...
- **x87 exceptions are reported but never delivered.** The status-word flags (IE/DE/ZE/OE/UE/PE) are set by arithmetic and validated against a real CPU, but `#MF` is never raised: a guest that *unmasks* an exception and expects a trap gets ES set and a silently-produced result instead. The stack-fault flag and the condition codes C0-C3 are still unmodelled, and a denormal operand does not raise DE (`TASK-328`). MXCSR is untouched on the SSE side — stored, reloaded, and governing nothing.
